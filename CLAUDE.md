...
- Toujours effectuer **2 appels TaxWare distincts** : un pour le domicile actuel, un pour le domicile cible, chacun avec son propre `Zip` et sa propre `City`.
- Ne jamais transporter un revenu imposable d'un canton à l'autre.
- Ne jamais réutiliser une base imposable d'un canton pour un autre.
- Chaque simulation doit être recalculée entièrement par TaxWare.
...
- Toujours vérifier que le payload envoyé correspond exactement à la structure attendue par TaxWare.
- Ne jamais simplifier ou modifier la structure du payload sans validation explicite.
...
- Toute modification doit être vérifiée sur le chemin complet : payload → réponse brute → normalisation → valeur affichée UI.
...
- Ne jamais appeler TaxWare directement depuis le frontend, même pour test ou debug.
